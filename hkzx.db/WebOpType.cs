...
{
    [Serializable]
    public class DataOpType : MSBaseData
    {
    ...
    //
    public class WebOpType
    {
        private const string TableName = "tb_Op_Type";
        SqlDAC sqlDac;
        public WebOpType()
        {
        ...
        //
        #region 查询
        public DataOpType[] GetData(int intId, string strFields = "")
        {
            ...
            DataOpType[] result = (DataOpType[])sqlDac.GetDataByAnyCondition(strSql, sqlParameters, typeof(DataOpType));
        ...
        public DataOpType[] GetDatas(int Active, string TypeName, string strFields = "", int intPage = 1, int pageSize = 0, string strOrderBy = "", string strFilter = "")
        {
            ...
            {
                list.Add(SqlParamHelper.AddParameter("@TypeName", SqlDbType.NVarChar, "TypeName", TypeName));
            ...
            DataOpType[] result = (DataOpType[])sqlDac.GetDataByAnyCondition(strSql, sqlParaArray, typeof(DataOpType));
            if (result != null && result.Length > 0)
            {
        ...
        private SqlParameter[] getParaArray(DataOpType data)
        {
            List<SqlParameter> list = new List<SqlParameter>();
